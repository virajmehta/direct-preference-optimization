...
def main(config: DictConfig):
    ...
    with open(f'/home/scratch/vdas/dpo/uncertainties/uncertainties_shp_shp_train_0.3_dropout_new.pkl','rb') as f:
        train_uncertainties = pickle.load(f)
    with open(f'/home/scratch/vdas/dpo/uncertainties/uncertainties_shp_shp_eval_0.3_dropout_new.pkl','rb') as f:
        eval_uncertainties = pickle.load(f)
    with open(f'/home/scratch/vdas/dpo/uncertainties/uncertainties_shp_jeopardy_0.3_dropout_new.pkl','rb') as f:
        jeo_uncertainties = pickle.load(f)
    sns.kdeplot(train_uncertainties, label=f'train')
    sns.kdeplot(eval_uncertainties, label=f'eval')
    sns.kdeplot(jeo_uncertainties, label=f'jeopardy')
    plt.legend()
    plt.xlim(-0.001, 0.005)
    plt.savefig(f'uncertainty_shp_0.3_dropout_final.png')
