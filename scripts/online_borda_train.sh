...
# python -u train_qlora.py model=phi2 datasets=[jokes] loss=sft exp_name=test gradient_accumulation_steps=1 batch_size=32 eval_batch_size=16 sample_during_eval=True active=False pretrain=True have_llm_dropout=True debug=True lr=1e-4
python -u train_qlora.py model=phi2 datasets=[jokes] loss=dpo loss.beta=0.1 exp_name=test gradient_accumulation_steps=1 batch_size=32 eval_batch_size=64  sample_during_eval=True online=True pretrain=False have_llm_dropout=True debug=True do_first_eval=False
#
